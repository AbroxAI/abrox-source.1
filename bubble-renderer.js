...
(function(){
  ...
  function init(){
    ...
    // ==== APPEND MESSAGE & HANDLE JUMP PILL ====
    let unseenCount = 0;
    ...
    function updateJump(){
      ...
      if(unseenCount <= 0){
        ...
        const span = jumpText.querySelector('span');
        if(span) span.remove();
        return;
      }

      if(unseenCount === 1){
        jumpText.textContent = 'New message';
        const span = jumpText.querySelector('span');
        if(span) span.remove();
      } else {
        jumpText.textContent = 'New messages ';
        let circle = jumpText.querySelector('span');
        if(!circle){
          circle = document.createElement('span');
          circle.style.background = 'var(--tg-accent)';
          circle.style.color = '#fff';
          circle.style.borderRadius = '50%';
          circle.style.padding = '0 8px';
          circle.style.marginLeft = '6px';
          circle.style.fontSize = '12px';
          circle.style.fontWeight = '600';
          circle.style.minWidth = '20px';
          circle.style.textAlign = 'center';
          jumpText.appendChild(circle);
        }
        circle.textContent = unseenCount;
      }
    }
