// bubble-renderer.js — Fixed Telegram-style bubble + adaptive SVG tail (`--tg-gap`) + avatars always visible
(function(){
  'use strict';
  ...
  function init(){
    ...
    // Read avatar size and gap from CSS variable
    let AVATAR_DIAM = 40;
    let GAP = 24; // default fallback for tail offset
    try{
      const style = getComputedStyle(document.documentElement);
      const v = style.getPropertyValue('--tg-avatar-size');
      const g = style.getPropertyValue('--tg-gap');
      if(v){ const parsed = parseInt(v.trim(),10); if(!Number.isNaN(parsed)&&parsed>0) AVATAR_DIAM=parsed; }
      if(g){ const parsedG = parseInt(g.trim(),10); if(!Number.isNaN(parsedG)&&parsedG>0) GAP=parsedG; }
    }catch(e){}
    ...
    function createBubbleElement(persona, text, opts={}){
      ...
      // adaptive tail via inline CSS variable
      content.style.setProperty('--tg-tail-offset', `${GAP}px`);
    ...
    console.log('bubble-renderer fully adaptive — tail offset follows CSS --tg-gap');
  }
